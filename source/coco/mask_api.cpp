...
void rleIou(RLE *dt, RLE *gt, siz m, siz n, byte *iscrowd, double *o) {
  ...
  db = new double[m*4];//malloc(sizeof(double)*m*4); 
  rleToBbox(dt,db,m);
  gb = new double[n*4]; //malloc(sizeof(double)*n*4); 
  rleToBbox(gt,gb,n);
...
void rleFrPoly( RLE *R, const double *xy, siz k, siz h, siz w ) {
  ...
  x = new int[k+1]; //malloc(sizeof(int)*(k+1)); 
  y = new int[k+1]; //malloc(sizeof(int)*(k+1));
  for(j=0; j<k; j++) x[j]=(int)(scale*xy[j*2+0]+.5); x[k]=x[0];
  for(j=0; j<k; j++) y[j]=(int)(scale*xy[j*2+1]+.5); y[k]=y[0];
  for(j=0; j<k; j++) m+=umax(abs(x[j]-x[j+1]),abs(y[j]-y[j+1]))+1;
  u = new int[m]; //malloc(sizeof(int)*m); 
  v = new int[m]; //malloc(sizeof(int)*m); m=0;
  m = 0;
  for( j=0; j<k; j++ ) {
  ...
  delete[] y;
  //free(x); free(y); 
  k=m; m=0; double xd, yd;
  x = new int[k]; //malloc(sizeof(int)*k); 
  y = new int[k]; //malloc(sizeof(int)*k);
  ...
  b = new uint[k]; //malloc(sizeof(uint)*k); 
  j=m=0; b[m++]=a[j++];
  while(j<k) if(a[j]>0) b[m++]=a[j++]; else {
    j++; if(j<k) b[m-1]+=a[j++]; }
  rleInit(R,h,w,m,b); 
  delete[] a;
  delete[] b;//free(a); free(b);
}
