...
namespace modeling{
  ...
  BoxCoder::BoxCoder(std::vector<float> weights, double bbox_xform_clip = log(1000. / 16))
                    :weights_(weights),
                     bbox_xform_clip_(bbox_xform_clip){}
  
  BoxCoder::encode(torch::Tensor reference_boxes, torch::Tensor proposals){
    int TO_REMOVE = 1;
    ...
    gt_widths = reference_boxes.slice(1, 2, 3) - reference_boxes.slice(1, 0, 1) + TO_REMOVE;
    gt_heights = reference_boxes.slice(1, 3, 4) - reference_boxes.slice(1, 1, 2) + TO_REMOVE;
    gt_ctr_x = reference_boxes.slice(1, 0, 1) + 0.5 * gt_widths;
    gt_crt_y = reference_boxes.slice(1, 1, 2) + 0.5 * gt_heights;

    return torch::stack({
      /*targets_dx*/ weights_[0] * (gt_crt_x - ex_ctr_x) / ex_widths,
      /*targets_dy*/ weights_[1] * (gt_crt_y - ex_ctr_y) / ex_heights,
      /*targets_dw*/ weights_[2] * torch::log(gt_widths / ex_widths),
      /*targets_dh*/ weights_[3] * torch::log(gt_heights / ex_heights)
    }, /*dim=*/1);
  }

  BoxCoder::decode(torch::Tensor rel_codes, torch::Tensor boxes){
    int TO_REMOVE = 1;
