...
std::pair<torch::Tensor, torch::Tensor> ConcatBoxPredictionLayers(std::vector<torch::Tensor>& box_cls, std::vector<torch::Tensor>& box_regression){
  ...
  for (decltype(box_cls.size()) i = 0; i < box_cls.size(); ++i) {
    ...
    //box_cls_per_level
    box_cls_flattened.push_back(PermuteAndFlatten(box_cls[i], N, A, C, H, W));
    box_regression_flattened.push_back(PermuteAndFlatten(box_regression[i], N, A, 4, H, W));
  }
