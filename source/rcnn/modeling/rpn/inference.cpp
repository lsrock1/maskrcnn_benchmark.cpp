...
std::vector<rcnn::structures::BoxList> RPNPostProcessorImpl::forward(std::vector<std::vector<rcnn::structures::BoxList>>& anchors, std::vector<torch::Tensor>& objectness, std::vector<torch::Tensor>& box_regression){
  ...
  assert(static_cast<int>(anchors_per_feature_maps.size()) == num_levels && static_cast<int>(anchors_per_feature_maps[0].size()) == num_imgs);
  ...
  for (decltype(sampled_boxes[0].size()) j = 0; j < sampled_boxes[0].size(); ++j) {
    bucket.clear();
