...
OSA_moduleImpl::OSA_moduleImpl(int64_t in_channels, 
  ...
  for (int i = 0; i < layers_per_block; ++i) {
    ...
    layers_.push_back(std::move(register_module("layers_" + std::to_string(i), seq)));
...
std::shared_ptr<torch::nn::Module> VoVNetImpl::clone(const torch::optional<torch::Device>& device) const {
