...
torch::Tensor PoolerImpl::ConvertToROIFormat(std::vector<rcnn::structures::BoxList> boxes){
  ...
  for (decltype(boxes.size()) i = 0; i < boxes.size(); ++i){
    ...
    ids_vec.push_back(torch::full({boxes[i].Length(), 1}, static_cast<int>(i), torch::TensorOptions().dtype(dtype).device(device)));
  }
...
torch::Tensor PoolerImpl::forward(std::vector<torch::Tensor> x, std::vector<rcnn::structures::BoxList> boxes) {
  auto num_levels = static_cast<int>(poolers_.size());
  ...
  auto output_size = std::get<0>(output_size_);
  ...
  for (decltype(poolers_.size()) level = 0; level < poolers_.size(); ++level) {
    torch::Tensor idx_in_level = torch::nonzero(levels == static_cast<int>(level)).squeeze(1);
    torch::Tensor rois_per_level = rois.index_select(/*dim=*/0, idx_in_level);
    result.index_copy_(/*dim=*/0, idx_in_level, poolers_[static_cast<int>(level)]->forward(/*per_level_feature=*/x[static_cast<int>(level)], rois_per_level).to(dtype));
  }
